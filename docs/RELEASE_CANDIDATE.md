...
3. **Fixed passwords removed**: every test previously logged in as the shared seed admin (`admin@iitjone.app` / the documented default password — confirmed still valid, contrary to an earlier assumption in this same investigation that it had been rotated). Replaced with `bootstrapTestAdmin()` (see §13), which creates a fresh admin with a random password per test run — no test anywhere in the suite depends on shared or fixed credentials anymore.
