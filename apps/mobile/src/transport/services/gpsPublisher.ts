...
// Module-scope, not a class field so listeners persist correctly.
...
class GpsPublisher {
  ...
  async start(): Promise<GpsPublisherStatus> {
    if (this.timer) return this.status; // already running — never publish twice concurrently
  ...
  /** Stops publishing immediately. Safe to call even if not currently active. */
